            ...
            size_t char_offset, size_t *entry_offset_byte_rtn )
{
    ...
    */
    ...
    int tracker = 0;
    uint8_t off_Seet = buffer->out_offs;
    ...
    size_t size__ = buffer->entry[buffer->out_offs].size;
   ...
   int first_iter_flag = 1;
    ...
    {      
       ...
       {
        for(int size = 1; size < buffer->entry[i].size ; size++)
        {
            if(trace < find)
            {
            trace = trace+1;
       ...
       {
         for(int size = 0; size < buffer->entry[i].size ; size++)
        {
            if(trace < find)
            {
            trace = trace+1;
    ...
    size_t return_offset = trace - size__;
    if(return_offset >=0 )
...
const char* aesd_circular_buffer_add_entry(struct aesd_circular_buffer *buffer, const struct aesd_buffer_entry *add_entry)
{
    ...
    */
    ...
    {
        //buffer is full
        return_buffer =  buffer->entry[buffer->out_offs].buffptr;
